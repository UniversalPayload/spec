...
.. table:: Node: /options/upl-params
   ...
   boot-mode         O           string list  For supporting different Payload boot flows. These influence how the payload runs. Note that this is not intended to support multiple Payloads.
                                              ...
                                              * “s3” means resuming from ACPI S3 state (resume from memory).
                                              ...
                                              Payload is not required to support all of the boot modes. For boot modes those are not supported by Payload, Payload can just ignore them without any specific actions.
   ...
   pci-enum-done     O           empty        When pci-enum-done present, payload will skip PCI resource assignment and it will use pci-rb node information to create internal resource map to ensure no conflict usage of those in-used PCI resources during payload execution.
                                              ...
                                              In some payloads, a simple PCI enumeration may still be executed even with “pci-enum-done” to collect PCI device information and bind corresponding drivers to provide services and functionality. 
...
.. table:: /pci/<pci-device>@<ecam-base-address>
   ...
   compatible        R           string list          To determine what kind of driver should be applied to the device. It should be in the form of <manufacturer>,<model>. But for historical reason,
                                                      <model> alone also works for common used devices, in this spec, "pci" is used for generic support.
   ...
   bus-range         R           prop-encoded-array   Contains 2 cells, each encoded as with encode-int, the first representing the bus number of the PCI bus implemented by the bus controller
...
4.2.6 Node: /isa (for IO Bus only) (O)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
This node is mandatory if a serial device is available, whether or not if it is
...
parameters chosen or specified by the system firmware at run time. It shall be a
...
.. table:: Node: chosen
   ...
   Property Name     Usage       Value Type   Definition
   ================= =========== ============ ======================================================================================================================================================================
   bootargs          O           string       A string that specifies the boot arguments for the client program. The value could potentially
                                              be a null string if no boot arguments are required.
   stdout-path       O           stringlist   A string that specifies the full path to the node representing the device (preferred) to be used
                                              by Payload for boot console and display output. If the character “:” is present in the value it
                                              terminates the path. The value may be an alias. If the stdin-path property is not specified,
                                              stdout-path should be assumed to define the input device.
   stdin-path        O           string       A string that specifies the full path to the node representing the device to be used for boot
                                              console input. If the character “:” is present in the value it terminates the path. The value
                                              may be an alias.
   ================= =========== ============ ======================================================================================================================================================================
...
Note: In this case the stdout is printed to two serial ports and one framebuffer
simultaneously.

4.2.9 Node: /options/upl-custom (O)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
